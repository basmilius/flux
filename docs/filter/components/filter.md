...
`defineFilter()` is a compile-time macro. Add the plugin from `@flux-ui/filter/vite` to your Vite config. Without it the call is left as a no-op and the filter will not register.
...
example=../../code/filter/components/filter/custom/preview.vue
:::
...
<<< @/code/filter/components/filter/custom/MyToggleFilter.vue [MyToggleFilter.vue]
<<< @/code/filter/components/filter/custom/preview.vue [Usage]
...
example=../../code/filter/components/filter/full.vue
:::

::: example Flyout || A filter that pops up when you press on a button.
example=../../code/filter/components/filter/flyout.vue
:::
