...
export function wrapFocus(container: HTMLElement, target: Element, forceFirst = false): void {
    ...
    (forceFirst || before ? (values[0] ?? container) : (values.at(-1) ?? container)).focus();
}
...
export function useFocusZone(target: RefObject<HTMLElement | null>, options: { direction?: 'horizontal' | 'vertical' | 'bidirectional'; ignore?: string } = {}): void {
    useEffect(() => {
        ...
        const key = (event: globalThis.KeyboardEvent) => {
            ...
            const next = getBidirectionalFocusElement(root, document.activeElement as HTMLElement, direction, options.ignore);
            if (next) {
                event.preventDefault();
                next.focus();
            }
    ...
    }, [target, options.direction, options.ignore]);
}
