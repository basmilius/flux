...
<script
    ...
    import { useEventsStore, useProjectsStore } from '@/store';
    import type { CalendarEvent } from '@/types';

    defineTitle('calendar', 'Calendar');

    const {events, reschedule} = useEventsStore();
    ...
    function onEvent(event: CalendarEvent): void {
        const project = event.projectId ? getProject(event.projectId)?.name : null;
        showSnackbar({icon: 'calendar', message: project ? `${event.title} · ${project}` : event.title});
    }
</script>
